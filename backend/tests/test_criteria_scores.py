...
def _register_and_login(
    client, db_session, email, password, full_name="Test", role="candidate", company_name=None
):
...
def _create_job(client, emp_headers):
    resp = client.post(
        ...
        json={
            "title": "Test Job",
            "description": "A test job posting with enough text.",
            "requirements": "Python",
            "benefits": "Remote",
            "job_type": "full_time",
...
class TestCriteriaScores:
    ...
    def test_bulk_replace_overwrites_old(self, client: TestClient, db_session: Session):
        ...
        client.put(
            ...
            json={
                "criteria": [
                    {"criteria_name": "A", "score": 5},
                    {"criteria_name": "B", "score": 7},
                ]
            },
            headers=emp,
        )
