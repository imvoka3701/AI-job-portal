...
def test_production_secret_key_validation():
    ...
    prod_settings = Settings(
        DEBUG=False, SECRET_KEY=strong_key, DATABASE_URL="postgresql://user:pass@localhost:5432/db"
    )
    assert prod_settings.DEBUG is False
    assert prod_settings.SECRET_KEY == strong_key

    # SQLite in-memory test mode -> does not block test execution
    sqlite_test_settings = Settings(
        DEBUG=False, SECRET_KEY="test-secret", DATABASE_URL="sqlite:///:memory:"
    )
