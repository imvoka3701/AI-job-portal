...
class CRUDCvDocument:
    def get_by_id(self, db: Session, *, document_id: int, user_id: int | None = None) -> CvDocument | None:
    ...
    def get_by_user(self, db: Session, *, user_id: int) -> list[CvDocument]:
        stmt = select(CvDocument).where(CvDocument.user_id == user_id).order_by(CvDocument.updated_at.desc())
