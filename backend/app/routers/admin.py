...
def _format_round_item(r: InterviewRound) -> dict:
    ...
    result_val = "passed" if r.status == "passed" else ("failed" if r.status == "failed" else "pending")
    return {
        "id": r.id,
...
def get_admin_stats(
    ...
    total_candidates = db.execute(
        text("SELECT COUNT(*) FROM users WHERE role = 'candidate'")
    ).scalar() or 0
    total_employers = db.execute(
        text("SELECT COUNT(*) FROM users WHERE role = 'employer'")
    ).scalar() or 0
    total_active_jobs = db.execute(
    ...
    new_users_last_30d = [
        {"date": str(r.day), "count": r.cnt} for r in new_users_rows
    ]
    ...
    new_applications_last_30d = [
        {"date": str(r.day), "count": r.cnt} for r in new_apps_rows
    ]

    # ── 4.5.8: candidate source ──────────────────────────────────────────
    direct_count = db.execute(
        text("SELECT COUNT(*) FROM users WHERE role='candidate' AND id NOT IN (SELECT DISTINCT user_id FROM oauth_accounts)")
    ).scalar() or 0
    oauth_count = db.execute(
        text("SELECT COUNT(*) FROM oauth_accounts WHERE provider='google'")
    ).scalar() or 0
...
def mark_interview_round_review(
    ...
    crud_admin_audit_log.create(
        ...
        action="interview_round.mark_review" if data.needs_review else "interview_round.clear_review",
