  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-medium text-emerald-600">CV Builder</p>
          <h1 className="text-3xl font-semibold text-gray-900">CV cá nhân của bạn</h1>
          <p className="mt-2 text-sm text-gray-600">Tạo CV chuyên nghiệp, lưu bản nháp và xuất PDF khi sẵn sàng.</p>
        </div>
        <Button onClick={create} isLoading={isCreating} leftIcon={<Plus className="h-4 w-4" />}>
          Tạo CV mới
        </Button>
      </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center py-16 text-center">
            ...
            <h2 className="mt-4 text-lg font-semibold text-gray-900">Bạn chưa có CV nào</h2>
            <p className="mt-2 max-w-md text-sm text-gray-500">
              Bắt đầu với một mẫu CV và hoàn thiện từng phần theo kinh nghiệm của bạn.
            </p>
            <Button className="mt-5" onClick={create} leftIcon={<Plus className="h-4 w-4" />}>
              Tạo CV đầu tiên
            </Button>
          </CardContent>
        </Card>
          ...
          >
            {documents.map((document) => (
              <motion.div
                ...
                className="flex flex-col gap-4 p-4 hover:bg-gray-50 transition-colors sm:flex-row sm:items-center sm:justify-between"
              >
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h2 className="truncate text-sm font-semibold text-gray-900">{document.title}</h2>
                      <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-600">
                        {document.status === "draft" ? "Bản nháp" : "Đã xuất bản"}
                      </span>
                    </div>
                    <p className="mt-1 flex items-center gap-1 text-xs text-gray-500">
                      <span>{CV_TEMPLATE_OPTIONS.find((item) => item.key === document.template_key)?.name ?? document.template_key}</span>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  <Link to={`/cv/${document.id}/edit`}>
                    <Button size="sm">Chỉnh sửa</Button>
                  </Link>
                  <Link to={`/cv/${document.id}/preview`}>
                    <Button size="sm" variant="outline">Xem trước</Button>
                  </Link>
                  <Button size="sm" variant="ghost" aria-label={`Nhân bản ${document.title}`} onClick={() => duplicate(document)} leftIcon={<Copy className="h-4 w-4" />} />
                  <Button size="sm" variant="ghost" aria-label={`Xóa ${document.title}`} onClick={() => remove(document)} leftIcon={<Trash2 className="h-4 w-4" />} />
                </div>
              </motion.div>
            ))}
          </motion.div>
